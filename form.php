...
if(!empty($_POST['send'])) {
	...
	$name = substr(htmlspecialchars(trim($_POST['fio'])), 0, 300);
	$tel = substr(htmlspecialchars(trim($_POST['tel'])), 0, 100);
    $email = substr(htmlspecialchars(trim($_POST['mail'])), 0, 100);
    $message = substr(htmlspecialchars(trim($_POST['txt'])), 0, 3000);
	$site = substr(htmlspecialchars(trim($_POST['site'])), 0, 300);
	...
	$mess .= "Телефон: <b>".$site."</b><br />";
	$mess .= "Комментарий: <b>".$message."</b><br />";
	//$mess .= "IP: <b>".$ip."</b><br />";
